...
//Beat Last Input Algorithm
RPSChoice BLI(){
  RPSChoice choice;
  choice = winningChoiceAgainst(arduinoChoices[roundsPlayed-1]); //THIS IS WHAT THE OPPONENT MIGHT DO!
  //choice = winningChoiceAgainst(choice); //THIS IS WHAT WE DO!
  return choice;
}

RPSChoice PlayLI(){
  RPSChoice choice;
  choice = arduinoChoices[roundsPlayed-1]; //THIS IS WHAT THE OPPONENT MIGHT DO!    FIXED: My bad lol - Mistake? Shouldn't it just be "choice = arduinoChoices[roundsPlayed-1];" ? -Terry
  //choice = winningChoiceAgainst(choice); //THIS IS WHAT WE DO!
  return choice;
}
...
RPSChoice WSLS(){
  RPSChoice choice;
  //If we win
  if(roundResults[roundsPlayed-1]==3){
    choice = winningChoiceAgainst(arduinoChoices[roundsPlayed-1]);
  }
  //If we Lose
  else if(roundResults[roundsPlayed-1]==1){
    choice = losingChoiceAgainst(arduinoChoices[roundsPlayed-1]);
  }  
  return choice;
}
...
RPSChoice makeRPSChoice(){
  ...
  */
  ...
  if (roundsPlayed == 0){
    ...
    RPSChoice opponent; //the predicted opponent's move
    float prob_opponent = 0; //the probability of the prediction being correct
    float prob_Rock = 0; //probability of OPPONENT using Rock next
    float prob_Paper = 0;
    float prob_Scissors = 0;
    float prob_BLI = 0; //probability of the opponent using BLI
    //add the probabilities of other strategy here//

    //counter Beat Last Input (BLI)
    int BLI_match = 0; //the number of opponent's moves that match the BLI pattern
    int Random = 0; // the number of moves considered random, might not be useful for everyone

    //counting elements that match and not match the BLI pattern
    for (int i = 1; i < roundsPlayed; i++){
        if(opponentChoices[i] == ((arduinoChoices[i-1] + 1) % 3)){
          BLI_match++;
        }else{
          Random++;
        }  
    ...
    //computing the probability of Random or not random
    Random++; //to account for the fact that the first opponent's move is assumed to be random
    int sum = BLI_match + Random;
    prob_BLI = (float) BLI_match / sum;
    RPSChoice BLI_Prediction; //predict opponent's move using BLI
    BLI_Prediction = BLI(); 

    //transfer the probability of BLI being used to the predicted variable
    if (BLI_Prediction == RPS_Rock){
      prob_Rock = prob_Rock + prob_BLI;
    } else if (BLI_Prediction == RPS_Paper){
      prob_Paper = prob_Paper + prob_BLI;
    } else if (BLI_Prediction == RPS_Scissors){
      prob_Scissors = prob_Scissors + prob_BLI;
    }

    //Repeat the above for each strategy to compute/add up the probability of the opponent's moves//
    
    //OWEN- Counter PlayLastInput ------START
    Random = 0; //need to be 0 before being used in each method -Terry
    int PlayLI_match = 0;
    for (int i = 1; i < roundsPlayed; i++){
        if(opponentChoices[i] == (arduinoChoices[i-1])){
          PlayLI_match++;
        }else{
          Random++;
        }  
    ...
    Random++; //to account for the fact that the first opponent's move is assumed to be random
    int sum = PlayLI_match + Random;
    prob_PlayLI = (float) PlayLI_match / sum;
    RPSChoice PlayLI_Prediction; //predict opponent's move using PlayLI
    PlayLI_Prediction = PlayLI(); 

    //transfer the probability of PlayLI being used to the predicted variable
    if (PlayLI_Prediction == RPS_Rock){
      prob_Rock = prob_Rock + prob_PlayLI;
    } else if (PlayLI_Prediction == RPS_Paper){
      prob_Paper = prob_Paper + prob_PlayLI;
    } else if (PlayLI_Prediction == RPS_Scissors){
      prob_Scissors = prob_Scissors + prob_PlayLI;
    }
    //OWEN- --------END

    //ARMAAN 5.7 Win-Stay-Lose-Shift Start
    Random = 0; 
    int WSLS_match = 0; 
    ...
    Random++; 
    sum = WSLS_match + Random;
    int prob_WSLS = (float) WSLS_match/sum;
    RPSChoice WSLS_Prediction; 
    WSLS_Prediction = WSLS(); 

    if (WSLS_Prediction == RPS_Rock){
      prob_Rock = prob_Rock + prob_WSLS;
    } else if (WSLS_Prediction == RPS_Paper){
      prob_Paper = prob_Paper + prob_WSLS;
    } else if (WSLS_Prediction == RPS_Scissors){
      prob_Scissors = prob_Scissors + prob_WSLS;
    }
    //ARMAAN END
    
    //code to handle equal probability
    if (prob_Rock == prob_Scissors || prob_Rock == prob_Paper){
      opponent = RPS_Rock; //This is arbitrary, statistically it does not matter
      prob_opponent = prob_Rock; 
    } else if (prob_Scissors == prob_Paper){
      opponent = RPS_Scissors;
      prob_opponent = prob_Scissors;
    }
    //Compare prob_Rock, prob_Paper, and prob_Scissors, then pick the highest//
    if (prob_Rock > prob_Scissors){
      opponent = RPS_Rock;
      prob_opponent = prob_Rock;
      ...
      opponent = RPS_Scissors;
      prob_opponent = prob_Scissors;
    } else if (prob_Paper > prob_Rock){
      opponent = RPS_Paper;
      prob_opponent = prob_Paper;
    } else if (prob_Paper > prob_Scissors){
      opponent = RPS_Paper;
      prob_opponent = prob_Paper;
    }

    //if the highest exceeds 0.5 (at least 50% chance of being right), then pick as opponent's move, else generate a random number for opponent's move//
    if (prob_opponent < 0.5){
      opponent = randRPS();
    }
    ...
    RPSChoice choice;
    choice = winningChoiceAgainst(opponent);
    return choice;
